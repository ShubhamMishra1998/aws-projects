package com.shubham.weather.bulkevent;
...
public class BulkEventLambda {
    ...
    public void handler(S3Event event){
       event.getRecords()
               .forEach(this::processS3EventRecord);

    }

    private void processS3EventRecord(S3EventNotification.S3EventNotificationRecord record) {
        final List<WeatherEvent> weatherEvents=readWeatherEventsFromS3(record.getS3().getBucket().getName(),
    ...
    private List<WeatherEvent> readWeatherEventsFromS3(String bucket, String key) throws RuntimeException {
        try {
